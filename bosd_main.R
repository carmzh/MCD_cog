# Import data ----
# script to analyse BOSD data, by pulling out test scores upon their first administration
# This is the master R script which is used to obtain results for BOSD paper
...
# 
# # Sz free, Sz Freq reduced, Sz Freq unchanged and NA
#  pre.sz.freq = names(pre.post.data)[grep("pre_.*_sz_freq",names(pre.post.data))]
#  post.sz.freq = names(pre.post.data)[grep("post_.*_sz_freq",names(pre.post.data))]
# # # surgical cases (N=36)
#  surg.inds = which(!is.na(pre.post.data[,"Date_of_surgery_1"]))
# # length(surg.inds)
# # pre surgery seizure free estimate available #CZ: do you mean pre surg seizure FREQ available?
# pre.surg.sz.freq.avail.inds = which(rowSums(!is.na(pre.post.data[surg.inds,pre.sz.freq]))>0)
# length(pre.surg.sz.freq.avail.inds)
# # post surgery seizure free estimate available #CZ: do you mean post surg seizure FREQ available?
# post.surg.sz.freq.avail.inds = which(rowSums(!is.na(pre.post.data[surg.inds,post.sz.freq]))>0)
# length(post.surg.sz.freq.avail.inds)
# # pre & post surgery seizure free estimate available #CZ: do you mean pre&post surg seizure FREQ available?
# pre.post.surg.sz.freq.avail.inds = which(rowSums(!is.na(pre.post.data[surg.inds,pre.sz.freq]))>0 & 
#                                            rowSums(!is.na(pre.post.data[surg.inds,post.sz.freq]))>0)
# length(pre.post.surg.sz.freq.avail.inds)
# # pre & post surgery seizure freq estimate available (double check)
# pre.post.surg.sz.freq.avail.inds.2 = intersect(pre.surg.sz.freq.avail.inds,post.surg.sz.freq.avail.inds)
# length(pre.post.surg.sz.freq.avail.inds.2)
# 
# ##### seizure free
# post.surg.sz.free.inds = which(rowMeans(pre.post.data[surg.inds,post.sz.freq],na.rm=TRUE)==0)
# # length(post.surg.sz.free.inds)
# # ##### not seizure free
#  post.surg.sz.ongoing.inds = which(rowMeans(pre.post.data[surg.inds,post.sz.freq],na.rm=TRUE)>0)
# # length(post.surg.sz.ongoing.inds)
# # ##### seizure reduction
# # post.surg.sz.reduction.inds = which(rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds],post.sz.freq] - 
# #                                                pre.post.data[surg.inds[post.surg.sz.ongoing.inds],pre.sz.freq],na.rm=TRUE) < 0)
# # length(post.surg.sz.reduction.inds)
# # # proportional reduction in each case
# prop.reduction = rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds[post.surg.sz.reduction.inds]],post.sz.freq],na.rm=TRUE)/
#   rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds[post.surg.sz.reduction.inds]],pre.sz.freq],na.rm=TRUE)
# # mean post-op sz frequency as proportion of pre-op seizure frequency among those with a reduction
# # # mean(prop.reduction)
# # min(prop.reduction)
# # max(prop.reduction)
# # ##### seizure unchanged
#  post.surg.sz.unchanged.inds = which(rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds],post.sz.freq],na.rm=TRUE) - 
#                                        rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds],pre.sz.freq],na.rm=TRUE) == 0)
#  length(post.surg.sz.unchanged.inds)
# # 
#  rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds[post.surg.sz.unchanged.inds]],pre.sz.freq],na.rm=TRUE)
#  rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds[post.surg.sz.unchanged.inds]],post.sz.freq],na.rm=TRUE)
# # 
# # ##### data NA to calculate change or post-surg freedom
# # # pre surgery seizure free estimate not available #CZ: do you mean pre surgery seizure FREQ estimate not available?
# # pre.surg.sz.freq.not.avail.inds = which(rowSums(!is.na(pre.post.data[surg.inds,pre.sz.freq]))==0)
# length(pre.surg.sz.freq.not.avail.inds)
# # post surgery seizure free estimate not available #CZ: do you mean pre surgery seizure FREQ estimate not available?
# post.surg.sz.freq.not.avail.inds = which(rowSums(!is.na(pre.post.data[surg.inds,post.sz.freq]))==0)
# length(post.surg.sz.freq.not.avail.inds)
# # intersect both pre and post surgery seizure freq estimate not available #CZ: is this just FYI?
# pre.post.surg.sz.freq.not.avail.inds = intersect(pre.surg.sz.freq.not.avail.inds,post.surg.sz.freq.not.avail.inds)
# length(pre.post.surg.sz.freq.not.avail.inds)
# # post.est available, pre.est not available #CZ: is this just FYI
# post.no.pre.surg.sz.freq.avail.inds = intersect(pre.surg.sz.freq.not.avail.inds,post.surg.sz.freq.avail.inds)
# length(post.no.pre.surg.sz.freq.avail.inds)
# # (either pre, or post-surg freq NA) & 
# post.surg.sz.change.unknown = which(is.na(rowMeans(pre.post.data[surg.inds,post.sz.freq],na.rm=TRUE) - 
#                                             rowMeans(pre.post.data[surg.inds,pre.sz.freq],na.rm=TRUE)) &
#                                       rowMeans(pre.post.data[surg.inds,post.sz.freq],na.rm=TRUE) != 0)
# length(post.surg.sz.change.unknown) # NA=0
# length(unique(c(pre.surg.sz.freq.not.avail.inds,post.surg.sz.freq.not.avail.inds))) # NA=11
# 
# #### Post-surg sz freq categories (take2)
# # Get post surgical counts of Sz Free, Sz Freq reduced, Sz Freq unchanged and NA
# pre.sz.freq = names(pre.post.data)[grep("pre_.*_sz_freq",names(pre.post.data))]
# post.sz.freq = names(pre.post.data)[grep("post_.*_sz_freq",names(pre.post.data))]
# # surgical cases
# surg.inds = which(!is.na(pre.post.data[,"Date_of_surgery_1"]))
# length(surg.inds)
# # pre surgery seizure free estimate available
# pre.surg.sz.freq.avail.inds = which(rowSums(!is.na(pre.post.data[surg.inds,pre.sz.freq]))>0)
# length(pre.surg.sz.freq.avail.inds)
# # post surgery seizure free estimate available
# post.surg.sz.freq.avail.inds = which(rowSums(!is.na(pre.post.data[surg.inds,post.sz.freq]))>0)
# length(post.surg.sz.freq.avail.inds)
# # pre & post surgery seizure free estimate available
# pre.post.surg.sz.freq.avail.inds = which(rowSums(!is.na(pre.post.data[surg.inds,pre.sz.freq]))>0 & 
#                                            rowSums(!is.na(pre.post.data[surg.inds,post.sz.freq]))>0)
# length(pre.post.surg.sz.freq.avail.inds)
# pre.post.surg.sz.freq.avail.inds.2 = intersect(pre.surg.sz.freq.avail.inds,post.surg.sz.freq.avail.inds)
# length(pre.post.surg.sz.freq.avail.inds.2)
# # post surgery seizure free
# post.surg.sz.free.inds = which(rowMeans(pre.post.data[surg.inds,post.sz.freq],na.rm=TRUE)==0)
# length(post.surg.sz.free.inds)
# # post-surgery not seizure free
# post.surg.sz.ongoing.inds = which(rowMeans(pre.post.data[surg.inds,post.sz.freq],na.rm=TRUE)>0)
# length(post.surg.sz.ongoing.inds)
# 
# # of those with ongoing seizures, how many nonetheless have a reduction
# post.surg.sz.reduction.inds = which(rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds],post.sz.freq] - 
#                                                pre.post.data[surg.inds[post.surg.sz.ongoing.inds],pre.sz.freq],na.rm=TRUE) < 0)
# length(post.surg.sz.reduction.inds)
# # report proportional reduction in each case
# prop.reduction = rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds[post.surg.sz.reduction.inds]],post.sz.freq],na.rm=TRUE)/
#   rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds[post.surg.sz.reduction.inds]],pre.sz.freq],na.rm=TRUE)
# # mean post-op sz frequency as proportion of pre-op seizure frequency among those with a reduction
# mean(prop.reduction)
...
#                                       rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds],pre.sz.freq],na.rm=TRUE) == 0)
# length(post.surg.sz.unchanged.inds)
# # mean across subjects who have unchanged seizures, pre-op
# rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds[post.surg.sz.unchanged.inds]],pre.sz.freq],na.rm=TRUE)
# # mean across subjects who have unchanged seizures, post-op
# rowMeans(pre.post.data[surg.inds[post.surg.sz.ongoing.inds[post.surg.sz.unchanged.inds]],post.sz.freq],na.rm=TRUE)
# 
# # get data unavailable to calculate change or post-surg freedom
# # pre surgery seizure free estimate not available
# pre.surg.sz.freq.not.avail.inds = which(rowSums(!is.na(pre.post.data[surg.inds,pre.sz.freq]))==0)
# length(pre.surg.sz.freq.not.avail.inds)
# # post surgery seizure free estimate available
# post.surg.sz.freq.not.avail.inds = which(rowSums(!is.na(pre.post.data[surg.inds,post.sz.freq]))==0)
# length(post.surg.sz.freq.not.avail.inds)
# # pre & post seizure freq estimate not available
# pre.post.surg.sz.freq.not.avail.inds = intersect(pre.surg.sz.freq.not.avail.inds,post.surg.sz.freq.not.avail.inds)
# length(pre.post.surg.sz.freq.not.avail.inds)
# # yes post but no pre surg sz freq available
# post.no.pre.surg.sz.freq.avail.inds = intersect(pre.surg.sz.freq.not.avail.inds,post.surg.sz.freq.avail.inds)
# length(post.no.pre.surg.sz.freq.avail.inds)
# post.surg.sz.change.unknown = which(is.na(rowMeans(pre.post.data[surg.inds,post.sz.freq],na.rm=TRUE) - 
#                                             rowMeans(pre.post.data[surg.inds,pre.sz.freq],na.rm=TRUE)) &
#                                       rowMeans(pre.post.data[surg.inds,post.sz.freq],na.rm=TRUE) != 0)
# length(post.surg.sz.change.unknown)
